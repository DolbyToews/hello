...
A1z = numpy.random.randint(low=z_low + 1, high=z_high, size =((k + c), 1))
A1 = numpy.concatenate((A1x, A1y, A1z), axis= 1)#matrix of positions of area one
...
A2z = numpy.random.randint(low=z_low + 1, high=z_high, size =((k + c), 1))
A2 = numpy.concatenate((A2x, A2y, A2z), axis= 1)#matrix of positions of area two
Energy_Matrix = numpy.random.uniform(low=0, high=1, size =((z - (y + 1)), (b - (a + 1)), (z_high - (z_low + 1))))#(y, x)
#includes energy values for cell membrane, but E[j] = math.inf will overwrite them
#doesn't include values for system boundary
...
for i in range(0, n):
        ...
        if numpy.array_equal(P[j], DC2):#note, this whole sequence will not work if there's only one particle
            ...
            if uph in y_cord_old[j] and h in y_cord[j]:
                if j in Reject_Type_One:
                    ...
                    E[j] = 0#in real testing the energy will not be 0, this is done here
                    #for testing ease
    ...
    for q in range(0, (k + c + g + f)): #prints out each particle into the output file
        ...
        if q == ((k + c + g + f) - 1): #checks if we're at the last particle
            print(file= fileout)
    s = numpy.vsplit(y_cord, [k, (k + c), (k + c + g), (k + c + g + f)])#splits up y-coordinates of particles
...
fileout.close() 
print(Energy_Matrix)
#print(Energy_Matrix[:, 0]) #[:, 0] gives first column, [0, ;] gives first row
print(Energy_Matrix[(z_cord[j] - (z_low + 1)), (y_cord[j] - (y + 1)), (x_cord[j] - (a + 1))])
#on the surface this looks wrong, but it is the right way to get the valid energy coordinate